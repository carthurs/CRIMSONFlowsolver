...
{
public:
	Netlist3DDomainReplacement(const int numberOfNetlistsUsedAsBoundaryConditions, const double oneResistanceToGiveEachResistor, const std::vector<double>& compliancesToGiveToCentralCapacitors, const double initialDomainPressure, const int hstep, const double generalizedAlphaMethodAlpha, const double delt, const std::map<int,int> mapFromZeroDSurfaceIndexToConnectedComponentIndex, const int startingTimestepIndex, const int numberOfDirichletBCTSurfaces, const int numberOfTimestepsBetweenRestarts)
	...
	m_startingTimestepIndex(startingTimestepIndex)
	{
		...
		mp_NetlistZeroDDomainCircuit = boost::shared_ptr<NetlistZeroDDomainCircuit> (new NetlistZeroDDomainCircuit(hstep, m_numberOfNetlistsUsedAsBoundaryConditions, thisIsARestartedSimulation, generalizedAlphaMethodAlpha, delt, oneResistanceToGiveEachResistor, compliancesToGiveToCentralCapacitors, initialDomainPressure, mapFromZeroDSurfaceIndexToConnectedComponentIndex, startingTimestepIndex, numberOfDirichletBCTSurfaces));
