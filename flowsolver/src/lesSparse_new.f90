...
subroutine fLesSparseApG(	col,	row,	pLhs,	 &
    ...
    !
    integer pntrb(nNodes), pntre(nNodes)
    !
    real*8	pisave
    integer	i,	j,	k
    !
    ...
    !call mkl_dcsrgemv('T', nNodes, -pLhs(1,1:col(nNodes+1)-1), col, row, p, q(:,1))
    call mkl_dcsrmv('t', nNodes, nNodes, -1.0D+00, 'g', pLhs(1,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p, 0.0D+00, q(:,1))

    !call mkl_dcsrgemv('T', nNodes, -pLhs(2,1:col(nNodes+1)-1), col, row, p, q(:,2))
    call mkl_dcsrmv('t', nNodes, nNodes, -1.0D+00, 'g', pLhs(2,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p, 0.0D+00, q(:,2))

    !call mkl_dcsrgemv('T', nNodes, -pLhs(3,1:col(nNodes+1)-1), col, row, p, q(:,3))
    call mkl_dcsrmv('t', nNodes, nNodes, -1.0D+00, 'g', pLhs(3,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p, 0.0D+00, q(:,3))
...
!    enddo
    ...
    call mkl_dcsrmv('t', nNodes, nNodes, -1.0D+00, 'g', pLhs(1,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,4), 0.0D+00, q(:,1))
    call mkl_dcsrmv('t', nNodes, nNodes, -1.0D+00, 'g', pLhs(2,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,4), 0.0D+00, q(:,2))
    call mkl_dcsrmv('t', nNodes, nNodes, -1.0D+00, 'g', pLhs(3,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,4), 0.0D+00, q(:,3))

    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(1,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,1), 1.0D+00, q(:,1))
    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(2,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,1), 1.0D+00, q(:,2))
    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(3,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,1), 1.0D+00, q(:,3))

    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(4,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,2), 1.0D+00, q(:,1))
    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(5,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,2), 1.0D+00, q(:,2))
    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(6,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,2), 1.0D+00, q(:,3))

    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(7,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,3), 1.0D+00, q(:,1))
    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(8,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,3), 1.0D+00, q(:,2))
    call mkl_dcsrmv('t', nNodes, nNodes, 1.0D+00, 'g', kLhs(9,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,3), 1.0D+00, q(:,3))
...
!    enddo
    ...
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(1,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,1), 0.0D+00, q)
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(2,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,2), 1.0D+00, q)
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(3,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,3), 1.0D+00, q)
...
!    enddo
    ...
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(2,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,2), 1.0D+00, q)
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(3,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,3), 1.0D+00, q)
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(4,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,4), 1.0D+00, q)
...
!    enddo
    ...
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(1,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,1), 1.0D+00, q(:,4))
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(2,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,2), 1.0D+00, q(:,4))
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(3,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,3), 1.0D+00, q(:,4))
    call mkl_dcsrmv('n', nNodes, nNodes, 1.0D+00, 'g', pLhs(4,1:col(nNodes+1)-1), row(1:col(nNodes+1)-1), pntrb, pntre, p(:,4), 1.0D+00, q(:,4))
