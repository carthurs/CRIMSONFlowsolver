module deformableWall
    ...
    type (i1d) :: mWNodes_EWB, mWNodes_EWB_gtlmap !nodes that are used in distance observations
...
contains
    ...
    subroutine vlmwStTri(x,     iBC,      BC)
        ...
        do iblk = 1, nelblb
            ...
            !
            !.... check if SWB, TWB, or EWB files are used
...
subroutine DdeformableWall
    ...
    if (ideformwall.ne.0) then
        do iblk = 1, nelblb
            ...
            if (associated(mEWB(iblk)%p)) deallocate (mEWB(iblk)%p)
