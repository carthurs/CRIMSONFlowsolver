...
TEST_F(testFileReaders, fileReaderGotFirstRCR) {
  ...
  EXPECT_EQ(3.815889462107935e-01, r1);

  double c =  rcrtReader_instance->getC()[0];
  EXPECT_EQ(1.885526988247284e-01, c);

  double r2 = rcrtReader_instance->getR2()[0];
  EXPECT_EQ(1.740713618386789e+00, r2);

  std::vector<std::pair<double,double>> timeData1 = rcrtReader_instance->getTimeDataPdist()[0];
  EXPECT_EQ(0.0,timeData1[0].first);
  EXPECT_EQ(0.0,timeData1[0].second);
  EXPECT_EQ(1.1,timeData1[1].first);
  EXPECT_EQ(0.0,timeData1[1].second);

}

// Tests that the fileReader reads the second entry of the test-case rcrt.dat
TEST_F(testFileReaders, fileReaderGotSecondRCR) {
  rcrtReader* rcrtReader_instance = rcrtReader::Instance();
  ...
  EXPECT_EQ(0.0,timeData2[0].first);
  EXPECT_EQ(0.0,timeData2[0].second);
  EXPECT_EQ(0.5,timeData2[1].first);
  EXPECT_EQ(1.7,timeData2[1].second);
  EXPECT_EQ(0.9,timeData2[2].first);
  EXPECT_EQ(2.3,timeData2[2].second);
}
