...
TEST_F(testMultidom, checkNetlistComponentNeighbourPointers)
{
	...
	for (auto component = netlistBC->mp_NetlistCircuit->mp_circuitData->components.begin(); component!= netlistBC->mp_NetlistCircuit->mp_circuitData->components.end(); component++)
	{
	...
	boost::weak_ptr<CircuitComponent> toPushOntoStack(netlistBC->mp_NetlistCircuit->mp_circuitData->mapOfComponents.at(indexOfComponentAt3DInterface));
	componentsNeedingChecking.push(toPushOntoStack);
	// To keep track of which components have been already checked:
	std::vector<bool> componentsWhichHaveBeenChecked(netlistBC->mp_NetlistCircuit->mp_circuitData->components.size(),false);
...
TEST_F(testMultidom, checkClosedDiodeWithRemainingOpenPathDetected)
{
	...
	EXPECT_TRUE(netlistBC_thirdNetlist->mp_NetlistCircuit->mp_circuitData->m_flowPermittedAcross3DInterface);
}
...
TEST_F(testMultidom, checkClosedDiodeWithoutRemainingOpenPathDetected)
{
	...
	EXPECT_FALSE(netlistBC_fourthNetlist->mp_NetlistCircuit->mp_circuitData->m_flowPermittedAcross3DInterface);
}