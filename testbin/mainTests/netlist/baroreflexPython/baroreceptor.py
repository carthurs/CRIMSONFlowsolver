...
class parameterController(abstractParameterController):
	...
	def __init__(self, baseNameOfThisScriptAndOfRelatedFlowOrPressureDatFile, MPIRank):
		...
		self.initialStates = numpy.array([initialHeartRateControlSignal, initialMaxElastanceControlSignal, initialPeripheralResistanceControlSignal, initialVenousComplianceControlSignal, initialVenousUnstressedVolumeControlSignal, initialHeartRateCentringValue, initialPeripheralResistanceCentringValue])
	...
	def updateControl(self, currentParameterValue, delt, dictionaryOfPressuresByNodeIndex, dictionaryOfFlowsByComponentIndex, dictionaryOfVolumesByComponentIndex):
		...
		if (self.baroreflexOn):
			...
			print "broadcasting ", [self.heartRate, states[1], states[2], states[3], states[4]], "not: ", states[5], states[6]
		else:
			...
			print "broadcasting ", [self.initialHeartRate, 1.0, 1.0, 1.0, 1.0], "not: ", states[5], states[6]

		if self.MPIRank == 0:
			self.fileWriteBuffer[self.fileWriteBufferNextWriteIndex, 0] = self.currentTimestepIndex
			self.fileWriteBuffer[self.fileWriteBufferNextWriteIndex, 1:-1] = states[0:-1]
			...
			self.fileWriteBuffer[self.fileWriteBufferNextWriteIndex, -1] = states[-1]
