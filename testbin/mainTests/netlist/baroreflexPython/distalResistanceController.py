...
class parameterController(abstractParameterController): #NECESSARY
	...
	def __init__(self, baseNameOfThisScriptAndOfRelatedFlowOrPressureDatFile, MPIRank): #NECESSARY
		...
		self.initialResistanceHasBeenSet = False
	...
	def updateControl(self, currentParameterValue, delt, dictionaryOfPressuresByNodeIndex, dictionaryOfFlowsByComponentIndex, dictionaryOfVolumesByComponentIndex): #NECESSARY
		...
		if (not self.initialResistanceHasBeenSet):
			self.baselineResistance = currentParameterValue
			self.initialResistanceHasBeenSet = True

		controlSignal = self.getRecievedBroadcastValue('baroreceptor','peripheralResistanceControlSignal')
		resistanceToSet = self.baselineResistance * controlSignal
		print "python setting resistance", resistanceToSet
